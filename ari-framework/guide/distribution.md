...
3. 결과물을 `~/.ari-agent/apps/<app_id>` 아래에 복사합니다.
4. 예시 배포 경로는 `~/.ari-agent/apps/notepad` 형태입니다.
5. ARIAgent를 재실행해서 새 앱과 `SKILL.md`를 다시 읽게 합니다.

> [!NOTE]
> 최신 ARIAgent는 사용자 설치 앱을 `~/.ari-agent/apps` 아래에서 우선 탐색합니다.
> `~/.ari-agent/skills`는 레거시 호환 경로로만 간주하세요.

**요청 예시:**
```text
이 프로젝트를 빌드해서 바이너리와 SKILL.md를 준비하고,
~/.ari-agent/apps/notepad 아래에 복사한 뒤,
ARIAgent를 재실행해줘.
```
...
- `~/.ari-agent/apps/<app_id>` 폴더가 생성되어 있어야 합니다.
