...
import { fileExistsSync, readTextSync } from "./data";
import { logger } from "./logger";
import { resolveServerPath } from "./runtime_paths";

export namespace Prompt {
  // 🔑 캐시: 템플릿 이름 → 마지막 수정 시각과 컴파일된 핸들바 함수
  const cache = new Map<string, { fullPath: string; mtimeMs: number; compiled: Handlebars.TemplateDelegate }>();
